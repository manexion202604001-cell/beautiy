...
import { CopyButton, Modal } from '@/components/client';
    ...
    <>
      ...
      <Modal open={open} onClose={() => setOpen(false)} title="個人情報のロック解除">
        {step === 'intro' ? (
          ...
          <StableActionForm<{ expiresAt: string }> action={verifyOtpAction} className="stack" showSuccess={false} onSuccess={() => { setOpen(false); setStep('intro'); router.refresh(); }}>
            ...
            <div className="between">
              ...
              <StableSubmit pendingText="確認中…">ロックを解除</StableSubmit>
            </div>
          </StableActionForm>
        )}
      </Modal>
...
export function TagEditor({ customerId, tags, allTags, canEdit }: {
  ...
  return (
    <div className="stack-sm">
      ...
      {canEdit && adding && (
        <StableActionForm action={addTagAction} onSuccess={() => { setAdding(false); setSel(''); }} showSuccess={false} resetOnSuccess>
          ...
          <div className="row-wrap">
            ...
            <StableSubmit className="btn sm" disabled={!sel}>追加</StableSubmit>
  ...
  return (
    <>
      ...
      <Modal open={open} onClose={() => setOpen(false)} title="ポイントの手動調整">
        <StableActionForm action={adjustPointsAction} onSuccess={() => setTimeout(() => setOpen(false), 900)}>
          <input type="hidden" name="customerId" value={customerId} />
          ...
          <div className="form-actions"><StableSubmit>調整する</StableSubmit></div>
        </StableActionForm>
      </Modal>
    </>
  );
  ...
  return (
    <>
      ...
      <Modal open={open} onClose={() => setOpen(false)} title="カウンセリングシートの記入依頼">
        <StableActionForm<{ url: string; status?: string }> action={issueCounselingAction} onSuccess={(r) => r.ok && setUrl(r.data?.url ?? null)}>
          <input type="hidden" name="customerId" value={customerId} />
          <div className="stack">
            <div className="field">
            ...
            <div className="form-actions"><StableSubmit>{url ? 'もう一度発行' : 'リンクを発行'}</StableSubmit></div>
          </div>
        </StableActionForm>
      </Modal>
    </>
