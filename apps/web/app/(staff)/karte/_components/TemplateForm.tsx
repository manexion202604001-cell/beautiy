'use client';
import { ModalButton } from '@/components/client';
...
export function TemplateButton({ template, label, className }: { template?: TemplateValues; label: React.ReactNode; className?: string }) {
  return (
    <ModalButton label={label} title={template?.id ? 'テンプレートを編集' : '新しいテンプレート'} className={className} wide>
      {(close) => (
        <StableActionForm action={saveTemplateAction} onSuccess={() => setTimeout(close, 600)}>
          ...
          <div className="form-actions"><button type="button" className="btn secondary" onClick={close}>キャンセル</button><StableSubmit>保存</StableSubmit></div>
        </StableActionForm>
      )}
    </ModalButton>
  );
